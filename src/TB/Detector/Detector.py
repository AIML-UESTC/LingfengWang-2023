...
from src.TB.Detector.nets.hourglass import get_hourglass
from src.TB.Detector.utils.utils import load_model
from src.TB.Detector.utils.image import transform_preds, get_affine_transform
from src.TB.Detector.utils.post_process import ctdet_decode
...
def inference(detection_model,classification_model,image):
  max_per_image = 100
  ...
  with torch.no_grad():
    ...
    for scale in imgs:
      ...
      output = detection_model(imgs[scale]['image'])[-1]
    ...
    for bbox in pred_bb:
      ...
      input = Variable(torch.unsqueeze(out_tensor, dim=0).float(), requires_grad=False).cuda()
    ...
    plt.savefig('./assets/TB_result.jpg')


def get_detector_model():
  #Load Detection model
  model = get_hourglass['large_hourglass']
  model = load_model(model, r'./ckpts/TB/detector.t7')
  model = model.cuda()
  model.eval()
