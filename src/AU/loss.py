...
class MultiLabelCircleLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        N,C = output.shape
        for i in range(C):
            loss = self.fn(output[:,i], target[:,i])
            if self.weights is not None:
                loss *= self.weights[i]
            total_loss += loss
        return total_loss

class CircleAULoss(nn.Module):
    """
    Lin's Concordance correlation coefficient
    """

    def __init__(self, ignore=-1,pos_weight = [1, 2, 1, 1, 1, 1, 1, 6, 6, 5, 1, 5]):
        super(CircleAULoss, self).__init__()
        self.ignore = ignore
        #[1, 2, 1, 1, 1, 1, 1, 6, 6, 5, 1, 5]
        #self.loss_fn = nn.BCEWithLogitsLoss()
        self.loss_fn = MultiLabelCircleLoss(weights=torch.Tensor(pos_weight).to(torch.cuda.current_device())).to(torch.cuda.current_device())
...
class DiceAULoss(nn.Module):
    ...
    def __init__(self, ignore=-1,pos_weight = [1, 2, 1, 1, 1, 1, 1, 6, 6, 5, 1, 5]):
        ...
        #[1, 2, 1, 1, 1, 1, 1, 6, 6, 5, 1, 5]
        #self.loss_fn = nn.BCEWithLogitsLoss()
        self.loss_fn = MultiLabelDiceLoss(weights=torch.Tensor(pos_weight).to(torch.cuda.current_device())).to(torch.cuda.current_device())
        self.loss_fn2 = torch.nn.BCEWithLogitsLoss(reduction='none', pos_weight=torch.Tensor(pos_weight).to(torch.cuda.current_device()))
...
class SmoothAULoss(nn.Module):
    ...
    def __init__(self, ignore=-1,pos_weight = [1, 1, 1, 1, 1, 1, 1, 3, 3, 3, 1, 2]):
        ...
        #[1, 2, 1, 1, 1, 1, 1, 6, 6, 5, 1, 5]
        #self.loss_fn = nn.BCEWithLogitsLoss()
        self.loss_fn = torch.nn.BCEWithLogitsLoss(reduction='none', pos_weight=torch.Tensor(pos_weight).to(torch.cuda.current_device()))
...
class FocalAULoss(nn.Module):
    ...
    def __init__(self, ignore=-1,pos_weight = [1, 1, 1, 1, 1, 1, 1, 5, 5, 5, 1, 4]):
        super(FocalAULoss, self).__init__()
        self.ignore = ignore
        #[1, 2, 1, 1, 1, 1, 1, 6, 6, 5, 1, 5]
        #self.loss_fn = nn.BCEWithLogitsLoss()
        self.loss_fn = torch.nn.BCEWithLogitsLoss(reduction='none', pos_weight=torch.Tensor(pos_weight).to(torch.cuda.current_device()))
