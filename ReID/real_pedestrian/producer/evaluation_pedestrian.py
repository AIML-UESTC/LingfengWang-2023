...
start_time = 0
receive_match = {}
pedestrain_gt_match,pedestrain_tem_list = {},{}
...
def visualize_precess(receive_match):
  global pedestrian_gt_match,pedestrian_tem_list
  target_trkid_dict = {}
  tem_trkid_dict = {}
  ...
  #可视化真实行人图片REID结果
  pedestrian_gt_match,pedestrian_tem_list
  for trkid1 in pedestrian_tem_list.keys():
    cnt = cnt + 1
    if receive_match[trkid1] == pedestrian_gt_match[trkid1]:
      corrects = corrects + 1
    target_img = cv2.imread(target_trkid_dict[trkid1])
    tem_img_list = []

    min_index = None
    for idx,pedestrian_tem_trkid in enumerate(pedestrian_tem_list[trkid1]):
      if pedestrian_tem_trkid == receive_match[trkid1]:
        min_index = idx
      tem_img_list.append(cv2.imread(tem_trkid_dict[pedestrian_tem_trkid]))
...
if __name__ == "__main__":
  ...
  # 发送真实车辆测试数据
  messages,pedestrian_gt_match,pedestrian_tem_list = generate_msgs('pedestrian')
  for message in messages:
    channel.basic_publish(exchange = '',routing_key = 'Coll2Reid',body = message)
  send_num = send_num + len(messages)
  print('Sending pedestrian: ',len(messages))
